...
# SQLite needs no pool tuning; MySQL benefits from pre-ping to survive dropped
# connections on managed cloud databases. A single streamed turn writes from two
# concurrent sessions (assistant message + live score) — on SQLite we give the
# writer a generous busy timeout so they never contend.
